from app.services.soe.factory import SOEFactory
from app.core.config import settings
from app.core.database import SessionLocal
from app.models.config import SystemConfig

def get_default_soe():
    """获取系统默认配置的 SOE 实例（从 SQLite 数据库实时读取）"""
    db = SessionLocal()
    try:
        ...
        import logging
        logging.error(f"Failed to read SOE config from DB: {e}")
        config = {"provider": "tencent", "providers": {}}
    finally:
        db.close()
    ...
    if provider == "tencent":
        ...
        return SOEFactory.get_service("tencent", secret_id=secret_id, secret_key=secret_key, appid=appid)
        
    elif provider == "xunfei":
        app_id = provider_settings.get("app_id")
        api_key = provider_settings.get("api_key")
        api_secret = provider_settings.get("api_secret")
        return SOEFactory.get_service("xunfei", app_id=app_id, api_key=api_key, api_secret=api_secret)
        
    return SOEFactory.get_service("tencent", secret_id=settings.tencent_secret_id, secret_key=settings.tencent_secret_key)
