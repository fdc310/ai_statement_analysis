...
严格按照系统提示的JSON格式输出。"""
    ...
    def _build_impromptu_reaction_system_prompt(self, language: str, has_scenario: bool) -> str:
        ...
        return f"""你是一名资深的即兴演讲与沟通教练。你的任务是针对"即兴反应"场景，结合用户的发言转写和语音词级时间戳，进行专业、犀利、结构化的评测。
...
{{
    ...
    "overall_scores": {{
        ...
        "speech_rate_level": "<优秀/良好/一般/较差>",
        "level": "<等级：优秀(85-100)/良好(70-84)/一般(55-69)/需改进(0-54)>",
        "one_sentence_comment": "<一句话总结，如：你成功连接了三个关键词，但主线略显松散，不超过30字>"
    }},
    "structure_visualization": {{
...
5. 表达精炼度(10%)：
   - 90-100: 无口头禅，表达干练，冗余度极低
   - 70-89: 偶有口头禅，基本精炼，冗余度正常
   - 50-69: 较多口头禅或冗余，冗余度偏高
   - 0-49: 大量废话，严重干扰，冗余度极高
...
**触发情境/题目**：{scenario}
**用户发言转写**：{text}
**语音词汇及时间戳信息**（可选参考）：{str(word_info_list)[:2000] if word_info_list else '无'}
...
你必须严格按照以下JSON格式输出，不要添加任何额外内容或 Markdown 格式块，只输出JSON：

{{
    "reaction_and_emotion": {{
        "score": <情绪与反应稳定性评分0-100>,
        "opening_speed": "<起步判断：果断开口/犹豫拖延/大量填充词起手>",
        "panic_signals": <是否存在明显慌乱(如语速突变、结巴)，true/false>,
        "emotion_assessment": "<起步与情绪表现的详细评价>"
    }},
    "structure_formation": {{
        "score": <结构表现评分0-100>,
        "formed_in_15s": <是否在开场(约前15秒)内建立主线结构，true/false>,
        "structure_pattern": "<实际表现出的结构，如'无序散发'或'总分总'>",
    ...
    "logic_and_relevance": {{
        ...
        "assessment": "<逻辑连贯性与切题度评价>"
    }},
    "expression_fluency": {{
        "score": <表达流畅度与精炼度评分0-100>,
        "filler_words": [
            {{"word": "<嗯/啊/然后/就是说等>", "count": <估算出现次数>}}
        ],
        "redundancy_level": "<冗余度判定：极低/正常/偏高/极高>",
        "assessment": "<表达流畅度及填充词比例评价>"
    }},
    "overall_scores": {{
        "overall_score": <综合评分0-100，由以上四项加权得出(如各25%)>,
        "reaction_score": <情绪与反应稳定性评分0-100>,
        "structure_score": <结构表现评分0-100>,
        "logic_score": <逻辑与切题度评分0-100>,
        "fluency_score": <表达流畅度与精炼度评分0-100>,
        "level": "<等级：优秀(85-100)/良好(70-84)/一般(55-69)/需改进(0-54)>",
        "one_sentence_comment": "<【一句话总结】如：你成功连接了三个关键词，但主线略显松散，不超过30字>"
    }},
    "next_action": "<【下一次只改一件事】给出唯一且最具操作性的改进建议，如：在开头先说清主线>",
